   ...
   public:
    ...
    void beginTuple() override {
        ...
        for (auto &cond : fed_conds_) {
            bool cond_col_is_index = false;
            // std::count(index_meta_.cols.begin(), index_meta_.cols.end(), cond.lhs_col.col_name) > 0;
            std::for_each(index_meta_.cols.begin(), index_meta_.cols.end(), [&](ColMeta &indexcols_metainfo) {
                indexcols_metainfo.name == cond.lhs_col.col_name ? cond_col_is_index = true : 0;
            });
            if (cond.is_rhs_val && cond.op != OP_NE && cond_col_is_index) {
