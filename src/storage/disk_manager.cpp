...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    off_t writehead = page_no * PAGE_SIZE;
    lseek(fd, writehead, SEEK_SET);
    auto write_rnt = write(fd, offset, num_bytes);
    if (write_rnt != num_bytes) throw InternalError("DiskManager::write_page Error");
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    off_t readhead = page_no * PAGE_SIZE;
    lseek(fd, readhead, SEEK_SET);
    auto read_rnt = read(fd, offset, num_bytes);
    if (read_rnt != num_bytes) throw InternalError("DiskManager::read_page Error");
...
void DiskManager::create_file(const std::string &path) {
    ...
    // const int kOPEN = 1;
    // if (path2fd_.count(path) == kOPEN) open(path.c_str(), O_CREAT);
    // else
    //  throw FileExistsError(path);

    const int kFOUND = true;
    if (is_file(path) == kFOUND) {
        throw FileExistsError(path);
    } else {
        open(path.c_str(), O_CREAT|O_RDWR,S_IRWXU);
    }
    return;
}
...
void DiskManager::destroy_file(const std::string &path) {
    ...
    const int kNOT_OPEN = 0;
    if (path2fd_.count(path) == kNOT_OPEN)
        unlink(path.c_str());
    else
        throw FileExistsError(path);
    return;
